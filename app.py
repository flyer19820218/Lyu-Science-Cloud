...
student_q = st.sidebar.text_input("打字問曉臻：", placeholder="例如：什麼是原子守恆？", key="science_q")
uploaded_file = st.sidebar.file_uploader("📸 照片區：", type=["jpg", "png", "jpeg"], key="science_f")

# --- 4. 曉臻教學 6 項核心指令 (真理對答案強化版) [cite: 2026-02-03] ---
SYSTEM_PROMPT = """
你是資深自然科學助教曉臻，馬拉松選手 (PB 92分)。語氣熱血專業。 [cite: 2026-02-01]

【教學指令 SOP】：
1. 【熱血開場】：隨機 30 秒聊體育賽評 (NBA、棒球) 或跑步心得，嚴禁唸字體顏色，必含『熱身一下下課老師就要去跑步了』。 [cite: 2026-02-03]
2. 【對答案協議】：若圖中有習題，必須先公佈答案，並假設全體同學都不會，啟動「分段配速解說」。 [cite: 2026-02-03]
3. 【上下文串連】：通讀全圖，將前面概念教學與後方練習題進行邏輯連結，用珍珠奶茶邏輯解釋。 [cite: 2026-02-01]
4. 【導航】：必說：『各位同學，請翻到第 X 頁。』 [cite: 2026-02-03]
5. 【轉譯】：LaTeX 公式轉口語 (如 n=m/M 唸作「莫耳數等於質量除以分子量」)。 [cite: 2026-02-03]
6. 【激勵】：結尾必喊『這就是自然科學的真理！』並鼓勵同學不要在馬拉松半路放棄。 [cite: 2026-02-03]
"""

# --- 5. 右側主畫面：頁碼直選(置頂) 與 能量域呈現 [cite: 2026-02-03] ---
st.title("🚀 臻·極速自然能量域 (實體課對應版)")
        ...
        if not user_key:
            ...
            with st.spinner("曉臻正在分析賽事戰報與對答案邏輯..."):
                try:
                    ...
                    prompt = f"{SYSTEM_PROMPT}\n請導讀第 {target_page} 頁。若有習題請先對答案並解說。"
